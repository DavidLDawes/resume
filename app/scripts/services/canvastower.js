...
angular.module('resumeApp')
    .factory('canvastower', ['$document', 'canvas', function ($document, canvas) {
        ...
        var battlefield, cBattle, ctxBattle, fight,
            imgDagger = $document[0].getElementById('dagger'),
            imgShortSword = $document[0].getElementById('shortsword'),
            imgLongSword = $document[0].getElementById('sword'),
            imgTwoHandedSword = $document[0].getElementById('twohandedsword'),
            imgBlank = $document[0].getElementById('blank'),
            statnames = ['st', 'dx', 'cn', 'hp', 'max', 'ac'],
            canvaswidth = 640, canvasheight = 200, fieldspaces = 40, xp = 20, yp = 20,
            xbattlefield = canvaswidth - 2 * xp,
            xspacesize = Math.floor(xbattlefield / fieldspaces),
            ybattlefield = canvaswidth - 2 * yp,
            yspacesize = ybattlefield,
            fightrounds = function (mefirst) {
            ...
            towercanvas = {
                'version': '0.0.1',
                'constants': {'fieldcount': fieldspaces, 'xsize': xspacesize, 'ysize': yspacesize, 'xporch': xp, 'yporch': yp},
                init: function () {
                ...
                drawFigures: function (bfield, drawctx) {
                    ...
                    //now draw the row
                    for (i = 0;   i < bfield.length; i = i + 1) {
                        if (bfield[i] === null) {
                            canvas.drawImage(xp + i * towercanvas.constants.x, yp + towercanvas.constants.x, imgBlank);
                        } else {
                            canvas.drawImage(xp + i * xspacesize, yp, imgLongSword);
                            for (stat = 0; stat < statnames.size; stat = stat + 1) {
                ...
                updateBattlefield: function (us, them) {
                    var i, found = -1;
                    for (i = 0; i < canvas.constants.width / 100; i = i + 1) {
                        // find our last one
                        ...
                        if (found < fieldspaces - 1) {
                            if (battlefield[found + 1] === null) {
                                ...
                                if (battlefield[found + 2] !== null) {
                                    ...
                                    for (i = found + 3; i < fieldspaces; i = i + 1) {battlefield[i - 1] = battlefield[i]; }
                                }
                            } else {
                ...
                hit: function (target, attacker) {
                    ...
                    if (Math.floor(Math.random() * 20) < attacker.thac0[target.ac]) {
                        ...
                        j = 0;
                        for (i = 0; i < attacker.weapon.damage.dice; i = i + 1) {
