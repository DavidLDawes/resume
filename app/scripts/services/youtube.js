...
angular.module('resumeApp')
  .factory('youtube', ['$http', function ($http) {
      var fetchSome;
      var OAUTH2_CLIENT_ID = 'AIzaSyCZjkovom0KStuGbfNP00v1b02Un-S1SSE';
      var OAUTH2_SCOPES = [
      ...
      var urlstart = 'https://www.googleapis.com/youtube/v3/search' +
          ...
          fetchSize;
      var urlend = '&q=vera&key=AIzaSyCZjkovom0KStuGbfNP00v1b02Un-S1SSE';
      ...
      fetchSome = function(now, token) {
          ...
          ytvideoPromise = $http.get(urlstart + insertMe + urlend).success(function (data) {
              ...
              console.log('********* status ********');
              for(index in status) {
                  var attr = status[index];
                  console.log(attr);
              }

              console.log('********* headers *******');
              for(index in headers) {
                  var attr = headers[index];
                  console.log(attr);
              }
              console.log('********* config ********');
